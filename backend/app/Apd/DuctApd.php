...
class DuctApd
{
    ...
    public static function getOptimalDimensions(string $shape, int $flowRate, int $secondSize=0, float $idealFlowSpeed = 7): float
    {
        ...
        if($shape === 'circular'){
            ...
            $optimaDimension = Diameter::where('diameter', '>=', $optimalDiameter)->first()->diameter;
        } elseif ($shape === 'rectangular') {
            $optimaDimension = round(($optimalSection / ($secondSize / 1000)) * 1000);
        }
        
        return round($optimaDimension, 3);
    }
    ...
    private function setLinearApd(): bool
    {
        ...
        $roughness = Material::where('name', $this->material)->first()->roughness;
        $b = 2.51 / $reynolds;
    ...
    private function setSingularApd(): bool
    {
        ...
        {
            $totalSingularities += Singularity::where(['name' => $singularityName, 'shape' => $this->shape])->first()->singularity * $singularityCount;
        }
